 ...
 * @version $Id: MapPNode.java 4180 2011-02-18 12:45:36Z mayer $
 */
public class MapPNode extends PNode {
    ...
    private void createDataImages() {
        final boolean imageDataEqualSized = state.getDataImagesMode() == SOMViewer.TOGGLE_DATA_IMAGES_EQUALLY;

        // FIXME: instead of remove and re-adding, create it once, and just do the resizing?
        imageDataNode.removeAllChildren();

        ArrayList<String> missingImages = new ArrayList<String>();
        Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                "Image SOM or imagePre/Suffx specified, adding layer of Images.");
        imageDataNode.moveToFront();

        Image missingImage = UiUtils.getIcon("cross.png").getImage();

        int numberOfMappedInputs = state.growingLayer.getNumberOfMappedInputs();
        ProgressListener progress = new StdErrProgressWriter(numberOfMappedInputs, "Loading data image ",
                numberOfMappedInputs / 10);

        int size = 0;
        int imageWidth = 0;
        int imageHeight = 0;
        if (imageDataEqualSized) {
            // find the unit with the most items, adjust size to that
            size = (int) Math.ceil(Math.sqrt(gsom.getLayer().getMaximumNumberOfMappedInputs()));
            imageWidth = UNIT_WIDTH / size;
            imageHeight = UNIT_HEIGHT / size;
        }

        String prefix = StringUtils.defaultIfBlank(CommonSOMViewerStateData.imagePrefix,
                CommonSOMViewerStateData.fileNamePrefix);
        String suffix = StringUtils.defaultIfBlank(CommonSOMViewerStateData.imageSuffix,
                CommonSOMViewerStateData.fileNameSuffix);

        for (GeneralUnitPNode[] unit : units) {
            for (GeneralUnitPNode element : unit) {
                if (element != null // check needed for mnemonic SOMs (might not have all units)
                        && element.getUnit().getNumberOfMappedInputs() > 0) {
                    Unit u = element.getUnit();
                    int items = u.getNumberOfMappedInputs();
                    int totalRows = (int) Math.ceil(items / (double) size);
                    if (state.getDataImagesMode() != SOMViewer.TOGGLE_DATA_IMAGES_EQUALLY) {
                        size = (int) Math.ceil(Math.sqrt(items));
                        imageWidth = UNIT_WIDTH / size;
                        imageHeight = UNIT_HEIGHT / size;
                    }
                    for (int index = 0; index < u.getNumberOfMappedInputs(); index++) {
                        String s = u.getMappedInputName(index);
                        File f = new File(prefix + s + suffix);
                        PImage img;
                        if (f.exists()) {
                            img = new PImage(f.getAbsolutePath());
                        } else {
                            img = new PImage(missingImage);
                            missingImages.add(f.getAbsolutePath());
                        }
                        String className = "";
                        if (classInfo != null) {
                            try {
                                className = "\nClass: " + classInfo.getClassName(s);
                            } catch (SOMLibFileFormatException e) {
                                e.printStackTrace();
                            }
                        }
                        img.addAttribute("tooltip", StringUtils.beautify(s) + className);

                        int offsetX = index % size;
                        int offsetY = index / size;
                        int xPos = u.getXPos() * UNIT_WIDTH + offsetX * imageWidth;
                        int yPos = u.getYPos() * UNIT_HEIGHT + offsetY * imageHeight;

                        if (imageDataEqualSized) {
                            // offset to centre the images vertically
                            int additionalRows = (size * size - items) / size;
                            yPos += additionalRows / 2.0 * imageHeight;

                            // centre images horizontally in the last data row
                            int currentRow = (int) Math.ceil((index + 1.0) / size);
                            if (currentRow == totalRows) {
                                int additionalColumns = (size * size - items) % size;
                                xPos += additionalColumns / 2.0 * imageWidth;
                            }
                        }

                        img.setBounds(xPos, yPos, imageWidth, imageHeight);
                        imageDataNode.addChild(img);
                        progress.progress();
                    }
                }
            }
        ...
        if (missingImages.size() > 0) {
            StringBuilder msg = new StringBuilder("Did not find not ").append(missingImages.size()).append(" images:");
            for (String string : missingImages) {
                msg.append("\n\t").append(string);
            }
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning(msg.toString());
            if (missingImages.size() == numberOfMappedInputs) {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info("No images found; hiding image layer");
                imageDataNode.setVisible(false);
            }
        }
    }
    ...
    private int getScaledBackgroundImageWidth(BackgroundImageVisualizer visualization) {
        ...
        return (int) (helper.getMapWidthInPx(UNIT_WIDTH, gsom.getLayer().getXSize()) / visualization.getPreferredScaleFactor());
    }
    ...
    private int getScaledBackgroundImageHeight(BackgroundImageVisualizer visualization) {
        GridGeometry helper = gsom.getLayer().getGridGeometry();
        return (int) (helper.getMapHeightInPx(UNIT_HEIGHT, gsom.getLayer().getYSize()) / visualization.getPreferredScaleFactor());
    }

    private int getBackgroundImageWidth() {
        GridGeometry helper = gsom.getLayer().getGridGeometry();
        return (int) helper.getMapWidthInPx(UNIT_WIDTH, gsom.getLayer().getXSize());
    }

    private int getBackgroundImageHeight() {
        GridGeometry helper = gsom.getLayer().getGridGeometry();
        return (int) (helper.getMapHeightInPx(UNIT_HEIGHT, gsom.getLayer().getYSize()));
    }
    ...
    public double getWidth() {
        ...
        return helper.getMapWidthInPx(UNIT_WIDTH, gsom.getLayer().getXSize());
    }
    ...
    public double getHeight() {
        GridGeometry helper = state.growingSOM.getLayer().getGridGeometry();
        return helper.getMapHeightInPx(UNIT_HEIGHT, gsom.getLayer().getYSize());
    }
    ...
    public void updateDataImagesVisibility() {
        if (state.getDataImagesMode() == SOMViewer.TOGGLE_DATA_IMAGES_NONE) {
            imageDataNode.setVisible(false);
        } else {
            createDataImages();
            imageDataNode.setVisible(true);
        }
    }

    public boolean hasDataImages() {
        return imageDataNode != null && imageDataNode.getChildrenCount() > 0;
    }
