...
import at.tuwien.ifs.commons.gui.controls.MultiOptionToggleButton;
import at.tuwien.ifs.commons.gui.controls.MultiOptionToggleListener;
import at.tuwien.ifs.commons.gui.util.MaximisedJFrame;
import at.tuwien.ifs.commons.util.io.ExtensionFileFilterSwing;
import at.tuwien.ifs.feature.ContentType;
 ...
 * @author Rudolf Mayer
 * @author Thomas Lidy
 * @version $Id: SOMViewer.java 4290 2014-01-05 17:36:06Z mayer $
 */
public class SOMViewer extends MaximisedJFrame implements ActionListener, Observer, SOMToolboxApp {
    ...
    public static final SOMToolboxApp.Type APPLICATION_TYPE = SOMToolboxApp.Type.Viewer;
    ...
    public static final Parameter[] OPTIONS = new Parameter[] {
            ...
            OptionFactory.getOptRegressionInformationFile(false),
            ...
            OptionFactory.getOptImagePrefix(), OptionFactory.getOptImageSuffix(),
    ...
    static final String TOGGLE_PIE_CHARTS_SHOW_CLASSNAME = "Pie-charts with class names";
    ...
    public static final String TOGGLE_DATA_IMAGES = "Show mapped images";

    public static final String TOGGLE_DATA_IMAGES_EQUALLY = "Show data images - equally sizes";

    public static final String TOGGLE_DATA_IMAGES_NONE = "No images";
    ...
    static {
        if (GraphicsEnvironment.isHeadless()) {
            ...
            Image ic = null;
            try {
                ic = Toolkit.getDefaultToolkit().getImage(
    ...
    public SOMViewer(JSAPResult config) throws HeadlessException {
        ...
        if (config.getString("imagePrefix") != null) {
            CommonSOMViewerStateData.imagePrefix = config.getString("imagePrefix");
        }
        if (config.getString("imageSuffix") != null) {
            CommonSOMViewerStateData.imageSuffix = config.getString("imageSuffix");
        }
    ...
    private void createAndShowGUI() {
        ...
        // if the map is trained on text data, also set document mode
        ContentType dataContentType = mapPane.getMap().getGsom().getDataContentType();
        if (dataContentType != null && dataContentType.isText()) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                    "Map file specified content type " + dataContentType + ", enabling document mode.");
            documentMode = true;
        }
        ...
        try {
            setIconImage(SOMViewer.APPLICATION_ICON);
        } catch (Exception e) {
        ...
        // Pie chart toggle button
        final String[] pieChartModeTexts = { TOGGLE_PIE_CHARTS_SHOW, TOGGLE_PIE_CHARTS_SHOW_COUNTS,
                TOGGLE_PIE_CHARTS_SHOW_PERCENT, TOGGLE_PIE_CHARTS_SHOW_CLASSNAME, TOGGLE_PIE_CHARTS_NONE };
        final ImageIcon pieChartModeIcons[] = { UiUtils.getIcon("piechart.png"), UiUtils.getIcon("piechart_label.png"),
                UiUtils.getIcon("piechart_label_percent.png"), UiUtils.getIcon("piechart_label_classname.png"),
                UiUtils.getIcon("piechart_off.png") };

        buttonPie = new MultiOptionToggleButton(pieChartModeIcons, pieChartModeTexts, "Toggle pie-chart display",
                new MultiOptionToggleListener() {
                    @Override
                    public void performAction(String actionCommand) {
                        state.setClassPiechartMode(actionCommand);
                        classLegendPane.setEnabled(true);
        ...
        // button to show/hide image representing data (on maps with content type image)
        final String[] dataImageModeTexts = { TOGGLE_DATA_IMAGES, TOGGLE_DATA_IMAGES_EQUALLY, TOGGLE_DATA_IMAGES_NONE };
        final ImageIcon dataImageModeIcons[] = { UiUtils.getIcon("photos.png"), UiUtils.getIcon("photos.png"),
                UiUtils.getIcon("photos.png") };

        MultiOptionToggleButton dataImagesToggleButton = new MultiOptionToggleButton(dataImageModeIcons,
                dataImageModeTexts, "Toggle data image display", new MultiOptionToggleListener() {
                    @Override
                    public void performAction(String actionCommand) {
                        state.setDataImagesMode(actionCommand);
                        getMap().updateDataImagesVisibility();
                    }
                });
        dataImagesToggleButton.setEnabled(getMap().hasDataImages());
        toolBar.add(dataImagesToggleButton);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (outputFile != null) {
                    try {
                        ...
                        JOptionPane.showMessageDialog(SOMViewer.this,
                                "Saving input corrections to file '" + outputFile.getAbsolutePath() + "' finished!");
                    } catch (SOMToolboxException ex) {
                        Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(ex.getMessage());
                    }
                }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (outputFile != null) {
                    try {
                        ...
                        JOptionPane.showMessageDialog(SOMViewer.this,
                                "Saving to unit file '" + outputFile.getAbsolutePath() + "' finished!");
    ...
    private void doAnimation() {
        ...
        for (Unit u : state.growingLayer.getAllUnits()) {
            ...
            AffineTransform transform = AffineTransform.getQuadrantRotateInstance(-rotatedQuadrants,
                    unit.getX() + unit.getWidth() / 2, unit.getY() + unit.getHeight() / 2);
            if (flippedX) {
    ...
    private JMenu createMapMenu() {
        ...
        JMenuItem rotCW = makeButtonMenutEntry("Rotate Clockwise", "map_rotate+90.png", 'R',
    ...
    private void displayFrame(AbstractSelectionPanel selectionPanel) {
        ...
        if (classColoursFile != null) {
            if (state.inputDataObjects.getClassInfo() != null) {
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox").warning(
                        "Class colour file " + classColoursFile
                                + " specified, but no class information file loaded - ignoring.");
            }
        }
        setVisible(true);
    }
    ...
    private void createHelpMenu() {
        ...
        final HelpDialog helpDialog = new HelpDialog(SOMViewer.this);
        ...
        JMenuItem itemAbout = new JMenuItem("About Java SOMToolbox");

        itemAbout.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                new AboutDialog(SOMViewer.this, "About Java SOMToolbox", INFO).setVisible(true);
            }
        });
    ...
    private void createVisualizationMenu() {
        ...
        JMenuItem clearVisCacheMenuItem = new JMenuItem("Visualisation Cache");
        clearVisCacheMenuItem.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                new VisualisationCacheViewer(SOMViewer.this).setVisible(true);
            }
        });
    ...
    private void createExportMenu() {
        ...
        exportVisualization.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if (getMap().getCurrentVisualization() == null) {
                    JOptionPane.showMessageDialog(getParent(), "No visualisation selected", "Error saving",
                            JOptionPane.ERROR_MESSAGE);
                } else {
                    JFileChooser fileChooser = SOMViewer.this.state.getFileChooser();
                    String defaultFileName = getMap().getCurrentVisualization().getDefaultVisualisationFileName(
                            state.currentVariant)
                            + ".png";
                    File filePath = ExportUtils.getFilePath(SOMViewer.this, fileChooser, "Save Visualization as PNG",
    ...
    public void update(Observable o, Object arg) {
        buttonPie.getMenu().setEnabled(false);
        if (arg instanceof SOMLibClassInformation && arg != null) { // we loaded a new class info
            ...
            c.fill = GridBagConstraints.BOTH;
            c.gridwidth = GridBagConstraints.REMAINDER;
            buttonPie.getMenu().setEnabled(true);
    ...
    private void initDocViewer(AbstractSelectionPanel sp) {
        /** ** DocViewPanel *** */
        DocViewPanel docviewer = new DocViewPanel(state.inputDataObjects);
        ...
        docViewerFrame = new JFrame("Document Viewer") {
            private static final long serialVersionUID = 1L;
