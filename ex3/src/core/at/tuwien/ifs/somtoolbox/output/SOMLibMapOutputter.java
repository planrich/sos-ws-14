 ...
 * @version $Id: SOMLibMapOutputter.java 4187 2011-03-04 12:43:50Z mayer $
 */
public class SOMLibMapOutputter {
    ...
    protected static void writeUnitInfoBegin(String fName, BufferedWriter bw, int j, int i, int k, int zSize)
            throws IOException {
        bw.write("$POS_X " + i);
        bw.newLine();
        bw.write("$POS_Y " + j);
    ...
    protected static void writeMappedVectors(BufferedWriter bw, String[] datalabels, double[] dataDistances)
            throws IOException {
    ...
    public static void writeMapDescriptionFile(GrowingSOM topLevelMap, String fDir, String fName,
        ...
        while (maps.size() > 0) { // process all remaining maps
            ...
            try {
                ...
                ContentType contentType = inputData.getContentType();
                mapDescription.setProperty(SOMLibMapDescription.DATA_TYPE, contentType.toString());
