 ...
 * @author Rudolf Mayer
 * @version $Id: ActivityHistogram.java 4331 2015-01-03 21:00:21Z mayer $
 */
class ActivityHistogram extends AbstractItemVisualizer {
    ...
    public ActivityHistogram() {
        ...
        setDefaultInterpolate(false);
    }

    @Override
    protected String getVisualisationSpecificCacheKey(int currentVariant) {
        return super.getVisualisationSpecificCacheKey(currentVariant) + CACHE_KEY_SECTION_SEPARATOR + "input:"
                + dataPoint;
    }
    ...
    public BufferedImage createVisualization(int index, GrowingSOM gsom, int width, int height)
        ...
        if (!(controlPanel instanceof ActivityHistrogramControlPanel) && !GraphicsEnvironment.isHeadless()) {
        ...
        return createImage(gsom, plane, width, height, isInterpolate());
    }
    ...
    private class ActivityHistrogramControlPanel extends AbstractSelectedItemVisualizerControlPanel implements
        ...
        private ActivityHistrogramControlPanel(ActivityHistogram hist, InputData inputData) {
            ...
            // if no state object present, get the singleton (can happen e.g. when we use the VisualisationImageSaver
            CommonSOMViewerStateData state = map != null ? map.getState() : CommonSOMViewerStateData.getInstance();
            listScroller.setPreferredSize(new Dimension(state.controlElementsWidth, 150));
            listScroller.setMaximumSize(new Dimension(state.controlElementsWidth, 150));
    ...
    public void setInputObjects(SharedSOMVisualisationData inputObjects) {
        ...
        if (inputData != inputObjects.getInputData()) {
            inputData = inputObjects.getInputData();
            if (inputData != null) {
                dataPoint = inputData.getInputDatum(0).getLabel();
            } else {
                dataPoint = null;
