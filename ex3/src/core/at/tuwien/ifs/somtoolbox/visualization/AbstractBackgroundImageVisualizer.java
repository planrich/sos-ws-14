 ...
 * @author Rudolf Mayer
 * @version $Id: AbstractBackgroundImageVisualizer.java 4328 2014-12-30 23:00:43Z mayer $
 */
public abstract class AbstractBackgroundImageVisualizer implements BackgroundImageVisualizer {
     ...
     */
    protected final String getCacheKey(GrowingSOM gsom, int currentVariant, int width, int height) {
        setInputObjects(gsom.getSharedInputObjects());
        String visSpecificKey = getVisualisationSpecificCacheKey(currentVariant);
        return getBasicCacheKey(gsom, currentVariant, width, height)
                + (visSpecificKey.trim().length() > 0 ? CACHE_KEY_SECTION_SEPARATOR + visSpecificKey : "");
    }

    @Override
    public final String getDefaultVisualisationFileName(int currentVariant) {
        String visSpecificKey = getVisualisationSpecificCacheKey(currentVariant);
        String key = getVisualizationShortName(currentVariant) + getOpacityCacheKeyComponent()
                + (visSpecificKey.trim().length() > 0 ? "__" + visSpecificKey : "");
        return escapeFileName(key);
    }

    public static final String getDefaultVisualisationFileName(String cacheKey) {
        String s = cacheKey;
        int beginCut = s.indexOf("SOM:");
        int indexOfOpacity = s.indexOf(CACHE_KEY_SECTION_SEPARATOR + "opac:");
        int endCut = indexOfOpacity != -1 ? indexOfOpacity : s.indexOf(CACHE_KEY_SECTION_SEPARATOR);
        endCut += CACHE_KEY_SECTION_SEPARATOR.length();
        return escapeFileName(s.substring(0, beginCut) + s.substring(endCut));
    }

    public static String escapeFileName(String key) {
        return key.replace(CACHE_KEY_SECTION_SEPARATOR, "__").replace(CACHE_KEY_SEPARATOR, "_").replace(":", "=");
    }
    ...
    private String getOpacityCacheKeyComponent() {
        return opacity != defaultOpacity ? CACHE_KEY_SECTION_SEPARATOR + "opac:" + opacity : "";
    }
    ...
    public static String buildCacheKey(Object... parts) {
        ...
        for (Object part : parts) {
            if (StringUtils.notEmpty(part) > 0) { // don't process empty parts
                if (sb.length() > 0) {
    ...
    public void setInputObjects(SharedSOMVisualisationData inputObjects) {
        if (this.inputObjects != inputObjects) {
    ...
    public class VisualizationControlPanel extends JPanel implements ComponentListener {
        ...
        public VisualizationControlPanel(String name) {
            ...
            if (AbstractBackgroundImageVisualizer.this instanceof MatrixVisualizer) {
                ...
                interpolateCheckbox.setSelected(((MatrixVisualizer) AbstractBackgroundImageVisualizer.this).isInterpolate());
                interpolateCheckbox.addActionListener(new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        ((MatrixVisualizer) AbstractBackgroundImageVisualizer.this).setInterpolate(interpolateCheckbox.isSelected());
                        if (visualizationUpdateListener != null) {
                            visualizationUpdateListener.updateVisualization();
                        }
                    }
