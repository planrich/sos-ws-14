...
public class GrowingLayer implements Layer {
    ...
    // TODO: remove this field and the usage of identifiers completely; covered by unitPath
    ...
    private final String revision = "$Revision: 4300 $";
    ...
    private int maximumNumberOfMappedInputs = -1;
    ...
    private void initSuperUnit(int id, Unit su) {
        ...
        if (su != null) {
            ...
            String superUnitPath = ((GrowingLayer) su.getLayer()).getUnitPath();
            if (level > 1) {
                unitPath = (superUnitPath.length() > 0 ? superUnitPath + "_" : "") + su.getXPos() + "x" + su.getYPos();
            }
        } else {
            level = 0;
        }
    ...
    public String getIdString() {
        if (getLevel() <= 1) {
            ...
            String id = "_submap_lvl" + getLevel() + "_";
            if (getSuperUnit() == null) {
                id += "0x0";
            } else {
                // append the path of units
                id += getUnitPath();
            }
            return id;
        }
    ...
    public QualityMeasure train(InputData data, double initialLearnrate, double initialSigma, int numIterations,
        ...
        double expParam = numIterations / 8.0; // TODO: hidden parameter
        double expParam2 = numIterations / 5.0; // TODO: hidden parameter
        ...
        if (targetQualityValue == Double.MAX_VALUE && tau < 1) { // to save time when using large data sets
            Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                    "Computing target quality: creating layer0, computing mean vector");
            ...
            if (System.currentTimeMillis() - startTime > 2 * 60 * 1000) { // 2 minutes
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                        "Finished computation of mean vector; starting computation of quality measure.");
            }
            ...
            if (System.currentTimeMillis() - startTime > 2 * 60 * 1000) { // 2 minutes
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info("Finished computation of quality measure.");
            }
        ...
        if (specialClassMode(trainingProps) || trainingProps.datumToUnitMappings().size() > 0) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").info("Performing special class training");

            /* Angela: special treatment of some clases during training */

            if (specialClassMode(trainingProps)) {
                try {
            ...
            if (trainingProps.getSelectedClassMode() == SelectedClassMode.EXCEPT) {
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                        "Skipping classes: " + trainingProps.getSelectedClasses() + ", total " + selectedInstances
                                + " inputs.");
            } else if (trainingProps.getSelectedClassMode() == SelectedClassMode.FAVOUR) {
    ...
    private void trainSpecial(InputData data, int numIterations, int startIteration, SOMProperties trainingProps,
        ...
        for (int i = startIteration; i < numIterations; i++) {
            if (til != null) {
                // don't interrupt in 1st iteration, to not mess up progress writer computation
                // FIXME: maybe write it before the progressWriter is initialised
                if (i > 0 && i % interruptEvery == 0) {
            ...
            try {
                ...
                if (contains && trainingProps.getSelectedClassMode() == SelectedClassMode.EXCEPT) {
                    ...
                    if (trainingProps.getSelectedClassMode() == SelectedClassMode.FAVOUR) {
    ...
    private void trainNormal(InputData data, int numIterations, int startIteration, SOMProperties trainingProps,
        ...
        for (int i = startIteration; i < numIterations; i++) {
            ...
            final double minLearnRate = 0.0001; // TODO: hidden parameter
            final double minSigma = 0.01; // TODO: hidden parameter
            ...
            if (trainingProps.batchSom()) {
                ...
                currentLearnrate = initialLearnrate * Math.exp(-1.0 * i / expParam); // exponential
                currentSigma = initialSigma * Math.exp(-1.0 * i / expParam2); // exponential
                if (currentLearnrate < minLearnRate) {
                    currentLearnrate = minLearnRate;
                }
    ...
    private boolean specialClassMode(SOMProperties trainingProps) {
        return trainingProps.getClassInfoFileName() != null
                && (trainingProps.getSelectedClassMode() == SelectedClassMode.EXCEPT || trainingProps.getSelectedClassMode() == SelectedClassMode.FAVOUR);
    }
     ...
     * 
     * @see InputContainer#getNumberOfMappedInputs()
     */
    public int getMaximumNumberOfMappedInputs() {
        if (maximumNumberOfMappedInputs == -1) {
            try {
                maximumNumberOfMappedInputs = 0;
                for (int j = 0; j < ySize; j++) {
                    for (int i = 0; i < xSize; i++) {
                        Unit unit = getUnit(i, j, 0);
                        if (unit != null && unit.getNumberOfMappedInputs() > maximumNumberOfMappedInputs) {
                            maximumNumberOfMappedInputs = unit.getNumberOfMappedInputs();
                        }
                    }
                }
            } catch (LayerAccessException e) {
                // doesn't happen, we're careful with the indices...
                e.printStackTrace();
            }
        }
        return maximumNumberOfMappedInputs;
    }
