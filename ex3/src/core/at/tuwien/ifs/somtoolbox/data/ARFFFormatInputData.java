 ...
 * 
 * @author Rudolf Mayer
 * @version $Id: ARFFFormatInputData.java 4273 2012-04-21 02:23:35Z mayer $
 */
public class ARFFFormatInputData extends SOMLibSparseInputData {
    ...
    public static final String REAL_TYPE = "REAL"; // used by Marsyas audio feature extraction
    ...
    protected void readVectorFile(String arffFileName, boolean sparse) {
        ...
        try {
            ...
            while (line != null && line.toUpperCase().startsWith(ATTRIBUTE)) {
                ...
                // check if the attribute name contains quotes
                int nameValueSplit;
                if (lineData.startsWith("'") || lineData.startsWith("\"")) {
                    if (lineData.startsWith("'")) {
                        nameValueSplit = lineData.indexOf("'", 1);
                    } else {
                        nameValueSplit = lineData.indexOf("\"", 1);
                    }
                } else {
                    nameValueSplit = lineData.indexOf(" ");
                }

                String name = lineData.substring(0, nameValueSplit).trim();
                name = StringUtils.stripQuotes(name);
                String type = lineData.substring(nameValueSplit).trim();

                // numerical attribute => treat as vector element
                if (INTEGER_TYPE.equalsIgnoreCase(type) || NUMERIC_TYPE.equalsIgnoreCase(type)
                        || REAL_TYPE.equalsIgnoreCase(type)) {
                    components.add(name);
                    ...
                    Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                            "Assuming categorical attribute '" + name + "' as class index, position " + classIndex);
                } else {
                    throw new IllegalArgumentException("Unknown attribute type '" + type + "' in line '" + line
                            + "' (line # " + lineNumber + ").");
                }
            ...
            while (line != null) {
                if (line.trim().length() > 0 && !line.trim().startsWith("%")) { // ignore empty lines && comments
                    lines.add(line.trim());
                }
                line = br.readLine();
                lineNumber++;
            ...
            for (String lineData : lines) {
                ...
                if (classIndex > 0) {
                    className = StringUtils.escapeString(values.get(classIndex));
